...
class Particle {
	pos = new Vector();
	posPrev = new Vector();
	vel = Vector.fa(random(TAU), random(0.5, 3));
	r = random(1, 7);
	color = random(360);
	created = performance.now();
	life = random(1, 3) * 1000;
}

class Firework {
	pos = new Vector(random(width), height);
	vel = Vector.fa(random(-1, 1) * EIGHTH_PI + PI * 1.5, random(4, 9));
	color = random(360);
	created = performance.now();
	life = random(0.5, 2.25) * 1000;
	particleCount = random(7, 12);
}