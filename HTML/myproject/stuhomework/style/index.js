        ...
        item.onclick = (e) => {//监听点击
            ...
            [...par.children].forEach((ele, index) => {//对于里面的内容也就是子元素
                if ([0, par.children.length - 1].indexOf(index) === -1) {//遍历去掉td和</td> 所以是从0- -1
                    ...
                    const input = `<input type="text" class="editable" value="${ele.innerHTML}" name="${ele.id}">`;//直接在里面打表单 innerHtml表示里面的文字
                    ele.innerHTML = input;
                }
            });
            par.lastElementChild.innerHTML = `<input type="submit" value="保存">`;
        }
    });
}